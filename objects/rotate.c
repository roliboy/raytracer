...
object rotate_y_create(object* inner, float angle) {
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            for (int k = 0; k < 2; k++) {
                float x = i * rotate_y_data.box.maximum.x + (1-i) * rotate_y_data.box.minimum.x;
                float y = j * rotate_y_data.box.maximum.y + (1-j) * rotate_y_data.box.minimum.y;
                float z = k * rotate_y_data.box.maximum.z + (1-k) * rotate_y_data.box.minimum.z;
