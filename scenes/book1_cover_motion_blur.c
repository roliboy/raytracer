        ...
        for (int b = -11; b < 11; b++) {
            ...
            if (vector_norm(vector_subtract(center, vector_create(4, 0.2, 0))) > 0.9) {
                if (choose_mat < 0.8) {
                    ...
                    vector center2 = vector_add(center, vector_create(0, random_float(0, 0.5), 0));
                    printf("moving_sphere %f %f %f %f %f %f %f %f %f diffuse %f %f %f\n", center.x, center.y, center.z, center2.x, center2.y, center2.z, 0.0, 1.0, 0.2, albedo.x, albedo.y, albedo.z);
