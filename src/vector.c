...
float vector_norm(vector v) { return sqrt(v.x * v.x + v.y * v.y + v.z * v.z); }
...
vector vector_normalize(vector vec) {
  ...
  return (vector){.x = vec.x / norm, .y = vec.y / norm, .z = vec.z / norm};
}
...
vector vector_refract(vector v, vector n, float eta) {
  ...
  vector r_out_perp = vector_multiply_scalar(
      vector_add(v, vector_multiply_scalar(n, cos_theta)), eta);

  vector r_out_parallel = vector_multiply_scalar(
      n, -sqrt(fabs(1.0 - vector_norm_squared(r_out_perp))));
...
vector vector_random_in_unit_sphere() {
  while (true) {
    vector random = vector_create(random_float(-1, 1), random_float(-1, 1),
                                  random_float(-1, 1));
    if (vector_norm_squared(random) >= 1)
      continue;
    return random;
  }
}
...
vector vector_random_on_unit_disk() {
  return vector_normalize(
      vector_create(random_float(-1, 1), random_float(-1, 1), 0));
}

vector vector_random_in_unit_disk() {
  while (true) {
    vector p = vector_create(random_float(-1, 1), random_float(-1, 1), 0);
    if (vector_norm_squared(p) >= 1)
      continue;
    return p;
  }
}
...
vector vector_random_in_range(float min, float max) {
  return (vector){.x = random_float(min, max),
...
vector vector_rgb(vector vec, int samples) {
  ...
  return (vector){.x = (float)(int)(256 * clamp(r, 0, 0.999)),
