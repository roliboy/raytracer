...
bool translate_hit(translate* tr, ray* r, float t_min, float t_max, hit* record) {
    ray moved_ray = ray_create(vector_subtract(r->origin, tr->offset), r->direction, r->time);
...
bool translate_bounding_box(translate* tr, float time0, float time1, bounding_box* box) {
    if (!object_bounding_box(tr->inner, time0, time1, box))
        return false;

    *box = bounding_box_create(
        vector_add(box->minimum, tr->offset),
