...
bounding_box bounding_box_create(vector minimum, vector maximum) {
  return (bounding_box){.minimum = minimum, .maximum = maximum};
}

bool bounding_box_hit(bounding_box *box, ray *r, float t_min, float t_max) {
  float t0, t1;
  t0 = fmin((box->minimum.x - r->origin.x) / r->direction.x,
            (box->maximum.x - r->origin.x) / r->direction.x);
  t1 = fmax((box->minimum.x - r->origin.x) / r->direction.x,
            (box->maximum.x - r->origin.x) / r->direction.x);
  t_min = fmax(t0, t_min);
  t_max = fmin(t1, t_max);
  if (t_max <= t_min)
  ...
  t0 = fmin((box->minimum.y - r->origin.y) / r->direction.y,
            (box->maximum.y - r->origin.y) / r->direction.y);
  t1 = fmax((box->minimum.y - r->origin.y) / r->direction.y,
            (box->maximum.y - r->origin.y) / r->direction.y);
  t_min = fmax(t0, t_min);
  t_max = fmin(t1, t_max);
  if (t_max <= t_min)
  ...
  t0 = fmin((box->minimum.z - r->origin.z) / r->direction.z,
            (box->maximum.z - r->origin.z) / r->direction.z);
  t1 = fmax((box->minimum.z - r->origin.z) / r->direction.z,
            (box->maximum.z - r->origin.z) / r->direction.z);
  t_min = fmax(t0, t_min);
  t_max = fmin(t1, t_max);
  if (t_max <= t_min)
...
bool bounding_box_hit_avx(bounding_box *box, ray *r, float t_min, float t_max) {
  ...
  _box = _mm256_set_ps(box->minimum.x, box->minimum.y, box->minimum.z, t_min,
                       box->maximum.x, box->maximum.y, box->maximum.z, t_max);
  _swapbox =
      _mm256_permutevar8x32_ps(_box, _mm256_set_epi32(3, 2, 1, 0, 7, 6, 5, 4));

  _origin = _mm256_set_ps(r->origin.x, r->origin.y, r->origin.z, 0, r->origin.x,
                          r->origin.y, r->origin.z, 0);
  _direction = _mm256_set_ps(r->direction.x, r->direction.y, r->direction.z, 1,
                             r->direction.x, r->direction.y, r->direction.z, 1);

  _box = _mm256_blendv_ps(_box, _swapbox,
...
bool bounding_box_hit_avx2(bounding_box *box, ray *r, float t_min,
                           float t_max) {
  __m256 _direction =
      _mm256_set_ps(r->direction.x, r->direction.y, r->direction.z, 1,
                    r->direction.x, r->direction.y, r->direction.z, 1);

  __m256 _box = _mm256_blendv_ps(
      _mm256_set_ps(box->minimum.x, box->minimum.y, box->minimum.z, t_min,
                    box->maximum.x, box->maximum.y, box->maximum.z, t_max),
      _mm256_set_ps(box->maximum.x, box->maximum.y, box->maximum.z, t_max,
                    box->minimum.x, box->minimum.y, box->minimum.z, t_min),
      _mm256_cmp_ps(_direction, _mm256_setzero_ps(), 1));

  __m256 _t = _mm256_div_ps(
      _mm256_sub_ps(_box,
                    _mm256_set_ps(r->origin.x, r->origin.y, r->origin.z, 0,
                                  r->origin.x, r->origin.y, r->origin.z, 0)),
      _direction);
...
bounding_box bounding_box_surround(bounding_box box0, bounding_box box1) {
  vector small = vector_create(fmin(box0.minimum.x, box1.minimum.x),
                               fmin(box0.minimum.y, box1.minimum.y),
                               fmin(box0.minimum.z, box1.minimum.z));

  vector large = vector_create(fmax(box0.maximum.x, box1.maximum.x),
                               fmax(box0.maximum.y, box1.maximum.y),
                               fmax(box0.maximum.z, box1.maximum.z));

  return (bounding_box){.minimum = small, .maximum = large};
}
...
int bounding_box_x_compare(const void *a, const void *b) {
  ...
  if (!object_bounding_box(*(object **)a, 0, 0, &box_a) ||
      !object_bounding_box(*(object **)b, 0, 0, &box_b)) {
    // puts("reeex");
    exit(-1);
  }
