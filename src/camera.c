...
camera camera_create(vector lookfrom, vector lookat, vector vup, float vfov,
  ...
  vector lower_left_corner = vector_subtract(
      origin, vector_add(vector_multiply_scalar(w, focus_dist),
                         vector_add(vector_divide_scalar(horizontal, 2),
...
ray camera_get_ray(camera *c, float s, float t) {
  vector rd =
      vector_multiply_scalar(vector_random_in_unit_disk(), c->lens_radius);
  vector offset = vector_add(vector_multiply_scalar(c->u, rd.x),
  ...
  return ray_create(
      ...
      vector_add(
          c->lower_left_corner,
          vector_add(vector_multiply_scalar(c->horizontal, s),
                     vector_subtract(vector_multiply_scalar(c->vertical, t),
                                     vector_add(c->origin, offset)))),
